...
fn generate_vite_env(schema: &AnubisSchema) {
  let content = format!(r#"/// <reference types="vite/client" />
...
"#), 
    &schema.install_directory.join("frontend/src/store/custom.ts"),
);
  ...
  for (i, model) in schema.models.iter().enumerate() {
    ...
    let reducer = format!("{model_name}: {slice}.reducer", model_name = model.name.to_lowercase(), slice = slice);
    let import = format!("import {{ slice as {slice} }} from '@/modules/{model_name}/reducer'\n", slice = slice, model_name = model.name.to_lowercase());
